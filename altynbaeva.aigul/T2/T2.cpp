...
struct DataStruct
{
    ...
    DataStruct() : type1(NONE), type2(NONE), double1(0), double2(0), ll1(0), ll2(0),
                   ull1(0), ull2(0), char1(0), char2(0) {}
};

bool parseDouble(const std::string &s, double &value)
{
    std::string str = s;
    if (!str.empty() && (str.back() == 'd' || str.back() == 'D'))
        str.pop_back();
    size_t pos = 0;
    try
    {
...
{
    if (s.size() < 6 || s.substr(0, 3) != "#c(" || s.back() != ')')
        return false;
    ...
    if (space == std::string::npos)
        return false;
    double real, imag;
    if (!parseDouble(inner.substr(0, space), real))
        return false;
    if (!parseDouble(inner.substr(space + 1), imag))
        return false;
...
{
    if (s.size() < 6 || s.substr(0, 3) != "(:N" || s.back() != ':')
        return false;
    ...
    if (!parseLongLong(numStr, num))
        return false;
    if (!parseUnsignedLongLong(denStr, den))
        return false;
    value = {num, den};
    return true;
}
...
std::istream &operator>>(std::istream &in, DataStruct &data)
{
    ...
    while (pos < line.size())
    {
        ...
        if (keyName == "key1")
        {
            if (parseChar(keyValue, d.char1))
            {
                d.type1 = DataStruct::CHAR;
                hasKey1 = true;
            }
            else if (parseDouble(keyValue, d.double1))
            {
                d.type1 = DataStruct::DOUBLE;
                hasKey1 = true;
            }
            else if (parseLongLong(keyValue, d.ll1))
            {
                d.type1 = DataStruct::LONG_LONG;
                hasKey1 = true;
            }
            else if (parseUnsignedLongLong(keyValue, d.ull1))
            {
                d.type1 = DataStruct::UNSIGNED_LONG_LONG;
                hasKey1 = true;
            }
            else if (parseComplex(keyValue, d.complex1))
            {
                d.type1 = DataStruct::COMPLEX;
                hasKey1 = true;
            }
            else if (parseRational(keyValue, d.rational1))
            {
                d.type1 = DataStruct::RATIONAL;
                hasKey1 = true;
            }
        }
        else if (keyName == "key2")
        {
            if (parseChar(keyValue, d.char2))
            {
                d.type2 = DataStruct::CHAR;
                hasKey2 = true;
            }
            else if (parseDouble(keyValue, d.double2))
            {
                d.type2 = DataStruct::DOUBLE;
                hasKey2 = true;
            }
            else if (parseLongLong(keyValue, d.ll2))
            {
                d.type2 = DataStruct::LONG_LONG;
                hasKey2 = true;
            }
            else if (parseUnsignedLongLong(keyValue, d.ull2))
            {
                d.type2 = DataStruct::UNSIGNED_LONG_LONG;
                hasKey2 = true;
            }
            else if (parseComplex(keyValue, d.complex2))
            {
                d.type2 = DataStruct::COMPLEX;
                hasKey2 = true;
            }
            else if (parseRational(keyValue, d.rational2))
            {
                d.type2 = DataStruct::RATIONAL;
                hasKey2 = true;
            }
        }
...
std::ostream &operator<<(std::ostream &out, const DataStruct &d)
{
    ...
    case DataStruct::DOUBLE:
        out << std::fixed << std::setprecision(1) << d.double1 << "d";
        break;
    ...
    case DataStruct::UNSIGNED_LONG_LONG:
        if (d.ull1 <= 9)
            ...
            out << "0x" << std::hex << std::uppercase << d.ull1 << std::dec;
        break;
    case DataStruct::CHAR:
        out << "'" << d.char1 << "'";
        break;
    case DataStruct::COMPLEX:
        out << "#c(" << d.complex1.real() << " " << d.complex1.imag() << ")";
        break;
    case DataStruct::RATIONAL:
        out << "(:N" << d.rational1.first << ":D" << d.rational1.second << ":)";
        break;
    default:
        break;
    }
    ...
    {
    case DataStruct::DOUBLE:
        out << std::fixed << std::setprecision(1) << d.double2 << "d";
        break;
    case DataStruct::LONG_LONG:
        out << d.ll2 << "ll";
        break;
    case DataStruct::UNSIGNED_LONG_LONG:
        if (d.ull2 <= 9)
            ...
            out << "0x" << std::hex << std::uppercase << d.ull2 << std::dec;
        break;
    case DataStruct::CHAR:
        out << "'" << d.char2 << "'";
        break;
    case DataStruct::COMPLEX:
        out << "#c(" << d.complex2.real() << " " << d.complex2.imag() << ")";
        break;
    case DataStruct::RATIONAL:
        out << "(:N" << d.rational2.first << ":D" << d.rational2.second << ":)";
        break;
    default:
        break;
    }
...
bool comparator(const DataStruct &a, const DataStruct &b)
{
    ...
    {
    case DataStruct::DOUBLE:
        return a.double1 < b.double1;
    case DataStruct::LONG_LONG:
        return a.ll1 < b.ll1;
    case DataStruct::UNSIGNED_LONG_LONG:
        return a.ull1 < b.ull1;
    case DataStruct::CHAR:
        return a.char1 < b.char1;
    case DataStruct::COMPLEX:
        return std::abs(a.complex1) < std::abs(b.complex1);
    case DataStruct::RATIONAL:
    {
