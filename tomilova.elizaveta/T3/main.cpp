...
namespace plg{
    ...
    std::istream& operator>>(std::istream& is, Polygon& pn){
        ...
        if(num_vertexes < 3){
            std::cerr << "Error: it is not polygon" << '\n';
            is.setstate(std::ios::failbit);
            return is;
        }
        pn.points.clear();
        pn.points.resize(num_vertexes);
        std::copy_n(std::istream_iterator<Point>(is), num_vertexes, pn.points.begin());
        return is;
    }
    ...
    void max(const std::vector<Polygon>& database, std::istream& is, std::ostream& os){
        ...
        if (argument == "AREA"){
            auto it =  std::max_element(database.begin(), database.end(), AreaComparator());
            os << std::fixed << std::setprecision(1) << getArea(*it) << "\n";
        }
        else if (argument == "VERTEXES"){
            auto it =  std::max_element(database.begin(), database.end(), VertexComparator());
            os << it->points.size() << "\n";;
        }
        else{
            throw std::logic_error("Invalid argument");
     ...
     void min(const std::vector<Polygon>& database, std::istream& is, std::ostream& os){
        ...
        if (argument == "AREA"){
            auto it =  std::min_element(database.begin(), database.end(), AreaComparator());
            os << std::fixed << std::setprecision(1) << getArea(*it) << "\n";
        }
        else if (argument == "VERTEXES"){
            auto it =  std::min_element(database.begin(), database.end(), VertexComparator());
            os << it->points.size() << "\n";;
        }
        else{
            throw std::logic_error("Invalid argument");
    ...
    void count(const std::vector<Polygon>& database, std::istream& is, std::ostream& os){
        ...
        if (argument == "EVEN"){
            double result =  std::count_if(database.begin(), database.end(), CountEven(true));
            os << result << "\n";
        }
        else if (argument == "ODD"){
            double result =  std::count_if(database.begin(), database.end(), CountEven(false));
            os << result << "\n";
        }
        else {
             try{
                ...
                double result = std::count_if(database.begin(), database.end(), CountVertex(num));
                os << result << "\n";
            }
            catch (...){
                throw std::logic_error("Invalid argument");
...
int main(int argc, char* argv[]){
    ...
    std::vector<Polygon> database;
        ...
        std::istream_iterator<Polygon>(), std::back_inserter(database));
