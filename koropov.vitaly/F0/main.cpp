...
int main()
{
  ...
  try
  {
    ...
    std::cout << "\nТекущая структура хеш-таблицы (переводы должны быть по алфавиту):\n";
    ...
    std::cout << "\nСтруктура таблицы после удаления (ячейка 'banana' должна стать [DELETED]):\n";
    dictionary.displayTable();

    std::cout << "\n=================================================================\n";
    std::cout << "ТЕСТ 4: Поиск слова после удаления (проверка сохранения цепочки коллизий)\n";
    std::cout << "=================================================================\n";
    std::cout << "Ищем слово 'book', чья цепочка пробирования проходит через удаленный слот 'banana'...\n";

    result = dictionary.search("book");
    if (result)
    {
      std::cout << "Успех! Алгоритм успешно прошел через ячейку [DELETED] и нашел элемент.\n";
    ...
    try
    {
      std::cout << "Пробуем добавить запись с пустым ключом: dictionary.insert(\"\", \"пустота\")...\n";
    ...
    std::cout << "\nТаблица ПЕРЕД критической вставкой (Размер: 4, Емкость: 5, Коэффициент: 0.8):\n";
    dictionary.displayTable();

    std::cout << "\nВставляем 5-е слово 'elephant'. Это должно спровоцировать РЕХЕШИРОВАНИЕ...\n";
    std::cout << "Алгоритм должен: \n";
    std::cout << "1. Удвоить емкость (5 * 1,5 = 7,5)\n";
    std::cout << "2. Найти ближайшее простое число (это 7)\n";
    std::cout << "3. Пересчитать новые хеш-индексы для ВСЕХ старых слов и очистить ячейки [DELETED]\n";
